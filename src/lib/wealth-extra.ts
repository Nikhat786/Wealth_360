...
export interface NominationAccount {
  id: string;
  name: string;
  institution: string;
  type: AccountType;
  value: number;
  /** Nominee already on record when Rahul joined, if any. */
  defaultNominee: string | null;
}

export const nominationAccounts: NominationAccount[] = [
  {
    id: "n1",
...
export interface TransferDoc {
  ...
  label: string;
  detail: string;
  /** Whether it is already in place at the start of the demo. */
  done: boolean;
}

export const transferDocs: TransferDoc[] = [
  {
    id: "d1",
