...
export type NominationMap = Record<string, NomineeEntry | null>;

export function defaultNominations(): NominationMap {
...
export function defaultDocs(): Record<string, boolean> {
...
export interface NominationSummary {
  ...
  totalValue: number;
  valuePct: number;
  missing: typeof nominationAccounts;
}

export function nominationSummary(map: NominationMap): NominationSummary {
...
export function transferScore(map: NominationMap, docs: Record<string, boolean>): number {
...
export const noExtraCover: CoverState = { extraLife: 0, extraHealth: 0, criticalIllness: 0 };

export function totalLifeCover(c: CoverState) {
  return insurance.lifeCover + c.extraLife;
}
export function totalHealthCover(c: CoverState) {
  return insurance.healthCover + c.extraHealth;
}

/** Standalone protection score, shown alongside the Wealth360 score. */
export function protectionScore(c: CoverState): number {
...
export function lifeCoverNeedsBreakdown(annualIncome: number, goalsShortfall: number) {
...
export interface DerivedAction {
  ...
  to: ActionRoute;
  cta: string;
  /** Rough score points on the line, used to rank. */
  weight: number;
}
...
export function buildActions(ctx: ActionContext): DerivedAction[] {
...
export function nextBestAction(actions: DerivedAction[]): DerivedAction | null {
