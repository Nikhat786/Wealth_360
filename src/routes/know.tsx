...
export const Route = createFileRoute("/know")({
  head: () => ({
    meta: [
      { title: "Know — your Wealth Map | m.Stock Wealth360" },
      {
        name: "description",
        content:
          "A single map of your person, family, cashflow, assets, liabilities, goals, protection and wealth transfer.",
      },
      { property: "og:title", content: "Know — your Wealth Map" },
      {
        property: "og:description",
        content: "Financial X-Ray of assets, debt, net worth, investments, insurance and goals.",
      },
    ],
  }),
  component: KnowPage,
...
function KnowPage() {
  const { answers, goals, goalsOffTrack, lifeCover, healthCover, dna, nominations_summary } =
    useApp();
  const income = derivedIncome(answers);
  const expenses = derivedExpenses(answers);
  const flows = cashflowTotals();
  const emi = answers.homeLoanEmi + answers.carLoanEmi + answers.personalLoanEmi;
  ...
  const nodes: MapNode[] = [
    {
      ...
      headline: `${answers.name}, ${answers.age}`,
      detail: `${answers.city} · ${answers.maritalStatus} · ${lifeStage(answers.age, answers.dependents)}`,
      tone: "navy",
    },
    {
      id: "family",
    ...
    },
    {
      ...
      detail: `${liabilities.map((l) => l.name).join(" and ")} · EMIs are ${debtToIncome(emi, income).toFixed(0)}% of income.`,
    ...
    },
    {
      ...
      detail: `Assessed need is ${formatINRShort(insurance.lifeCoverNeeded)} of life cover and ${formatINRShort(insurance.healthCoverNeeded)} of health cover.`,
      tone: lifeCover < insurance.lifeCoverNeeded ? "destructive" : "success",
    },
    {
      id: "transfer",
      layer: "Wealth transfer",
      icon: mapIcons.transfer,
      headline: `${nominations_summary.covered}/${nominations_summary.total} accounts have a nominee`,
          ...
          <div className="surface-card mt-4 overflow-x-auto">
            <table className="w-full min-w-[640px] text-sm">
              ...
              <tbody className="divide-y">
                {allocation.map((a) => {
                  const p = assetProfile[a.name] ?? { liquidity: "Medium" as const, risk: "Moderate" as const };
                  return (
                    <tr key={a.name}>
