...
export interface FamilyMember {
  id: string;
  name: string;
  relation: "Spouse" | "Son" | "Daughter" | "Mother" | "Father";
  age: number;
  dependent: boolean;
  note: string;
}
...
export const lifeStage = (age: number, dependents: number) => {
...
export const assetProfile: Record<string, { liquidity: "High" | "Medium" | "Low"; risk: "Low" | "Moderate" | "High" }> = {
...
export const debtToIncome = (emi: number, income: number) => (income > 0 ? (emi / income) * 100 : 0);
...
export function readinessScore(goals: Goal[]): number {
...
export const targetAllocations: Record<RiskProfile, Record<string, number>> = {
...
export function rebalancePlan(profile: RiskProfile): RebalanceRow[] {
...
export interface TaxOpportunity {
  ...
  lockIn: string;
  risk: "Low" | "Moderate" | "Market-linked";
  note: string;
}

export const taxOpportunities: TaxOpportunity[] = [
  {
    id: "t-elss",
    section: "80C",
    name: "ELSS top-up",
    headroom: taxSaving.section80cLimit - taxSaving.section80cUsed,
    estimatedSaving: Math.round((taxSaving.section80cLimit - taxSaving.section80cUsed) * 0.312),
    lockIn: "3 years",
    risk: "Market-linked",
    note: "Equity-linked. Also feeds your wealth-creation goal.",
  },
  {
    id: "t-nps",
    section: "80CCD(1B)",
    name: "NPS additional contribution",
    headroom: taxSaving.nps80ccdLimit - taxSaving.nps80ccdUsed,
    estimatedSaving: Math.round((taxSaving.nps80ccdLimit - taxSaving.nps80ccdUsed) * 0.312),
    lockIn: "Till age 60",
    risk: "Market-linked",
    note: "Retirement-locked, so it only suits money you will not need earlier.",
  },
  {
    id: "t-80d",
    section: "80D",
    name: "Health premium / top-up cover",
    headroom: taxSaving.healthPremium80dLimit - taxSaving.healthPremium80dUsed,
    estimatedSaving: Math.round((taxSaving.healthPremium80dLimit - taxSaving.healthPremium80dUsed) * 0.312),
    lockIn: "Annual",
    risk: "Low",
    note: "Closes a protection gap and reduces tax at the same time.",
  },
  {
    id: "t-home",
    section: "24(b)",
    name: "Home loan interest already claimed",
    headroom: 0,
    estimatedSaving: 62400,
    lockIn: "While the loan runs",
    risk: "Low",
    note: "Fully used this year. No further action needed.",
  },
];
...
export function debtStrategies(surplus: number): DebtStrategy[] {
  const base = 3120000;
...
export function inflationImpact(goal: Goal, inflation = 6) {
...
export interface CompareOption {
  ...
  provider: string;
  benefit: string;
  risk: "Low" | "Moderate" | "High" | "Market-linked";
  liquidity: string;
  tenure: string;
  cost: string;
  suitability: string;
...
export interface AutopilotAlert {
  ...
  title: string;
  detail: string;
  tone: "gold" | "destructive" | "success" | "primary";
}

export const autopilotAlerts: AutopilotAlert[] = [
...
export interface CalendarEvent {
  ...
  month: string;
  title: string;
  kind: "Maturity" | "Renewal" | "Tax" | "Review" | "EMI" | "Goal";
}

export const wealthCalendar: CalendarEvent[] = [
...
export const emptySim: SimulatorState = {
  sipDelta: 0,
  prepay: 0,
  careerBreakMonths: 0,
  inflationDelta: 0,
...
export function simulate(sim: SimulatorState, base: { surplus: number; goalsOnTrack: number; goalsTotal: number }): SimulatorResult {
...
export const vaultFinancial = [
  { id: "v1", label: "Demat & equity", detail: `${holdings.filter((h) => h.category === "Equity").length} holdings · m.Stock`, value: holdings.filter((h) => h.category === "Equity").reduce((s, h) => s + h.current, 0) },
  { id: "v2", label: "Mutual funds", detail: "SIPs across 4 schemes", value: holdings.filter((h) => h.category === "Mutual Funds").reduce((s, h) => s + h.current, 0) },
  { id: "v3", label: "Bank & deposits", detail: "HDFC savings + 2 FDs", value: holdings.filter((h) => h.category === "Fixed Deposits" || h.category === "Cash").reduce((s, h) => s + h.current, 0) },
  { id: "v4", label: "Retirement", detail: "EPF + NPS Tier-I", value: holdings.filter((h) => h.category === "EPF & NPS").reduce((s, h) => s + h.current, 0) },
  { id: "v5", label: "Insurance", detail: `Life ${(insurance.lifeCover / 10000000).toFixed(1)} Cr · Health ₹7 L`, value: insurance.lifeCover },
  { id: "v6", label: "Loans", detail: `${liabilities.length} active loans`, value: -totalLiabilities },
];
...
export const actionMeta: Record<string, { why: string; assumptions: string[]; confidence: "High" | "Medium" | "Low"; risk: "Low" | "Moderate" | "Market-linked"; marketLinked: boolean; horizon: "Today" | "This year" | "Later" }> = {
...
export const netWorthSnapshot = { totalAssets, totalLiabilities, netWorth: totalAssets - totalLiabilities };

export const baseGoalIds = baseGoals.map((g) => g.id);
export const cashflowBase = monthlyCashflow;
