...
export const Route = createFileRoute("/wealth360/review")({
  component: Wealth360Review,
});

const sections = ["ABOUT YOU", "FAMILY", "CASHFLOW", "INVESTMENTS", "LIABILITIES", "GOALS", "PROTECTION", "RISK"];

function Wealth360Review() {
  ...
  const totalAssets = answers.equity + answers.mutualFunds + answers.deposits + answers.gold + answers.retirementCorpus + answers.cashSavings + answers.propertyValue;
  const totalLiabilities = answers.liabilities.reduce((total, liability) => total + liability.outstandingAmount, 0) || 4650000;

  const build = () => {
    completeOnboarding(answers);
    void navigate({ to: "/wealth360/analysis" });
  };
    ...
    <AppShell minimal>
      <div className="mx-auto max-w-3xl space-y-6 py-6 sm:py-12">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          <Summary label="About you" value={`${answers.name}, ${answers.age} · ${answers.maritalStatus}`} detail={`${answers.dependents} dependents`} />
          ...
          <Summary label="Protection" value={`${formatINRShort(answers.lifeCover)} life cover`} detail={`${formatINRShort(answers.healthCover)} health cover`} />
          <Summary label="Risk" value="Moderate" detail={`${answers.horizon}-year investment horizon`} />
        </div>
